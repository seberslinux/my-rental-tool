...
const { getOne, getAll, run, closeDb } = require('../src/db/database');
...
async function main() {
  ...
  const existing = await getOne('SELECT id FROM properties WHERE smoobu_id = $1', [SMOOBU_ID]);
  if (existing) {
    await run('DELETE FROM bookings WHERE property_id = $1', [existing.id]);
    await run('DELETE FROM reviews WHERE property_id = $1', [existing.id]);
    await run('DELETE FROM expenses WHERE property_id = $1', [existing.id]);
    await run('DELETE FROM cleaning_jobs WHERE property_id = $1', [existing.id]);
    await run('DELETE FROM daily_rates WHERE property_id = $1', [existing.id]);
    await run('DELETE FROM property_costs WHERE property_id = $1', [existing.id]);
    await run('DELETE FROM properties WHERE id = $1', [existing.id]);
    console.log('Cleaned up previous test data.');
  }
  // Also remove old "Oceanview Cottage (Test)" if it exists
  const oldTest = await getOne("SELECT id FROM properties WHERE name LIKE '%Oceanview%Test%'");
  if (oldTest) {
    await run('DELETE FROM bookings WHERE property_id = $1', [oldTest.id]);
    await run('DELETE FROM reviews WHERE property_id = $1', [oldTest.id]);
    await run('DELETE FROM expenses WHERE property_id = $1', [oldTest.id]);
    await run('DELETE FROM cleaning_jobs WHERE property_id = $1', [oldTest.id]);
    await run('DELETE FROM daily_rates WHERE property_id = $1', [oldTest.id]);
    await run('DELETE FROM property_costs WHERE property_id = $1', [oldTest.id]);
    await run('DELETE FROM properties WHERE id = $1', [oldTest.id]);
    console.log('Cleaned up old Oceanview test property.');
  }

  // --- Create property ---
  const propResult = await run(`
  ...
  while (cursor < endDate) {
    ...
    while (dayPointer <= daysInMonth) {
      const los = pickLos();
      if (dayPointer + los > daysInMonth + 3) break;
  ...
  // --- Reviews ---
  const pastBookings = await getAll(`
    SELECT smoobu_id, guest_name, check_out, platform, language FROM bookings
    WHERE property_id = $1 AND status = 'confirmed' AND check_out < NOW()
  ...
  for (const b of pastBookings) {
    ...
    `, [propId, b.platform, b.guest_name, rating, comment, reviewDate, response, `test_${b.smoobu_id}`, b.language || 'en', sentiment]);
    reviewCount++;
  }
  ...
  while (expMonth < new Date('2026-04-01')) {
    ...
    for (const e of monthlyExp) {
      ...
      `, [propId, e.category, amount, e.desc, dateStr, 1, 'monthly', 'ZAR']);
      expenseCount++;
    }
  ...
  for (const e of oneOffs) {
    await run(`
      INSERT INTO expenses (property_id, category, amount, description, expense_date, recurring, recurring_frequency, currency)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
    `, [propId, e.category, e.amount, e.desc, e.date, 0, '', 'ZAR']);
    expenseCount++;
  }
  ...
  const stats = await getOne(`
    SELECT COUNT(*) as cnt, ROUND(AVG(total_price)) as avg_price, ROUND(AVG(length_of_stay)::numeric, 1) as avg_los, ROUND(AVG(price_per_night)) as avg_ppn
    FROM bookings WHERE property_id = $1 AND status = 'confirmed'
  `, [propId]);
  ...
  const all = await getAll(`
    SELECT p.name, COUNT(b.id) as bookings, ROUND(AVG(b.total_price)) as avg_price,
      ROUND(AVG(b.length_of_stay)::numeric, 1) as avg_los, ROUND(AVG(b.price_per_night)) as avg_ppn
